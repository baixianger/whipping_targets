...
class ReplayBuffer:
    ...
    def add(self, state, action, next_state, reward, done):
        """Add (S A S' R D) to replay buffer"""
    ...
    def sample(self, batch_size):
        """Sample a batch of (S A S' R D) from replay buffer"""
        if batch_size > self.size:
            raise ValueError(
                f"The size of replay buffer must be larger than batch size{batch_size}!!")
        ind = np.random.randint(0, self.size, size=batch_size)
        return (
            torch.as_tensor(
            	self.state[ind],      device=self.device, dtype=torch.float32),
            torch.as_tensor(
                self.action[ind],     device=self.device, dtype=torch.float32),
            torch.as_tensor(
                self.next_state[ind], device=self.device, dtype=torch.float32),
            torch.as_tensor(
                self.reward[ind],     device=self.device, dtype=torch.float32),
            torch.as_tensor(
                self.not_done[ind],   device=self.device, dtype=torch.float32),
        )


class VPGBuffer:
    """
    A buffer for storing trajectories experienced by 
    a VPG or PPO agent interacting with the environment, 
    ...
    def __init__(self, state_dim, action_dim, max_size, gamma=0.99, lam=0.95, is_discrete=False):
        ...
        self.action_buf = np.zeros((max_size, action_dim), dtype=np.float32)\
            if not is_discrete else np.zeros(max_size, dtype=np.float32)
    ...
    def statistics_scalar(x):
        """calculate mean and std for a list of numbers."""
    ...
    def _get_samples(self, batch_inds: np.ndarray):
        ...
        advantage_buf = (advantage_buf - advantage_mean) / \
            (advantage_std + 1e-8)
        return (
            torch.as_tensor(self.state_buf[batch_inds],
                            device=self.device, dtype=torch.float32),
            torch.as_tensor(self.action_buf[batch_inds],
                            device=self.device, dtype=torch.float32),
            torch.as_tensor(self.reward_to_go_buf[batch_inds],
                            device=self.device, dtype=torch.float32),
            torch.as_tensor(advantage_buf,
                            device=self.device, dtype=torch.float32),
            torch.as_tensor(self.logp_buf[batch_inds],
                            device=self.device, dtype=torch.float32),
        )
