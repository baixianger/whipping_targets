...
class DiscreteActor(nn.Module):
    ...
    def forward(self, state, action=None):
        ...
        # log(prob_i) = log(e^i / sum(e^j))
        logp_all = F.log_softmax(logits_a, dim=-1)
        ...
        if action is None:
            ...
            logp_a = torch.sum(
                F.one_hot(action.long(), num_classes=self.action_dim) * logp_all, dim=-1)
        logp_pi = torch.sum(
            F.one_hot(pi, num_classes=self.action_dim) * logp_all, dim=-1)
        return pi, logp_pi, logp_a


class ContinuousActor(nn.Module):
    """Policy network for continuous action space.
    在实现中,μ(s)通过神经网络来拟合,而σ(s)有两种常见的做法:
    1. 也用神经网络来拟合,与μ(s)的网络共享一些层。此时σ(s)能够根据状态s的不同而变化。
    2. 不做拟合,直接设置为一个常数,如1.0。此时σ(s)不依赖于状态s,保持不变。"""
    ...
    def __init__(
        ...
        self.log_std = nn.Parameter(torch.as_tensor(
            log_std, dtype=torch.float32))  # transfer to trained params
        self.EPS = 1e-8
        self.apply(weight_init)
...
class PPO(nn.Module):
    ...
    def select_action(self, state, deterministic=False):
        state = torch.as_tensor(state.reshape(
        ...
        for i in range(train_pi_iters):
            ...
            for state, action, g_reward, adv_value, logp_old in replay_buffer.sample():
                ...
                clip_adv = torch.clamp(
                    ratio, 1-self.clip_ratio, 1+self.clip_ratio) * adv_value
                ...
                if self.target_kl is not None and approx_kl > 1.5 * self.target_kl:
                    print(
                        f"Early stopping at updating step {i} / {train_pi_iters} due to reaching max kl {approx_kl:.2f}.")
        ...
        for _ in range(train_v_iters):
            ...
            for state, action, g_reward, _, _ in replay_buffer.sample():
                ...
                critic_loss = F.mse_loss(
                    current_V, torch.unsqueeze(g_reward, dim=1))
                # Optimize the critic
    ...
    def load(self, filename):
        self.critic.load_state_dict(torch.load(
            filename + "_critic", map_location=device))
        self.actor.load_state_dict(torch.load(
            filename + "_actor", map_location=device))
